...
public:
    ...
    BFS_2D(int length, int width, int height);
    ~BFS_2D();
...
inline bool BFS_2D::inBounds(int x, int y, int z) const
{
    return !(x < 0 || y < 0 || z < 0 ||
...
template <typename InputIt>
void BFS_2D::run(InputIt cells_begin, InputIt cells_end)
{
    if (m_running) {
        return;
    }
...
inline int BFS_2D::getNode(int x, int y, int z) const
{
    if (!inBounds(x, y, z)) {
        return -1;
    }
...
inline bool BFS_2D::getCoord(int node, int& x, int& y, int& z) const
{
